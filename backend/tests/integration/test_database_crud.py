...
class TestCustomerCRUD:
    ...
    @pytest.mark.asyncio
    async def test_create_customer(self, session: AsyncSession):
    ...
    @pytest.mark.asyncio
    async def test_get_customer(self, session: AsyncSession, test_customer: Customer):
        """Test retrieving customer by ID."""
        customer = await get_customer(session, test_customer.id)
    ...
    @pytest.mark.asyncio
    async def test_get_customer_not_found(self, session: AsyncSession):
        """Test retrieving non-existent customer returns None."""
        customer = await get_customer(session, uuid4())

        assert customer is None

    @pytest.mark.asyncio
    async def test_update_customer(self, session: AsyncSession, test_customer: Customer):
    ...
    @pytest.mark.asyncio
    async def test_delete_customer(self, session: AsyncSession, test_customer: Customer):
        """Test deleting a customer."""
        deleted = await delete_customer(session, test_customer.id)
    ...
    async def test_list_customers(
        self, session: AsyncSession, test_customer: Customer, test_customer_premium: Customer
    ):
        """Test listing customers with pagination."""
        customers = await list_customers(session, limit=10, offset=0)
...
class TestCustomerIdentifierCRUD:
    ...
    @pytest.mark.asyncio
    async def test_create_customer_identifier(self, session: AsyncSession, test_customer: Customer):
    ...
    async def test_get_customer_by_identifier_email(
        self, session: AsyncSession, test_customer: Customer, test_customer_identifier_email: CustomerIdentifier
    ):
    ...
    async def test_get_customer_by_identifier_phone(
        self, session: AsyncSession, test_customer: Customer, test_customer_identifier_phone: CustomerIdentifier
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_customer_by_identifier_not_found(self, session: AsyncSession):
    ...
    async def test_list_customer_identifiers(
        ...
        identifiers = await list_customer_identifiers(session, test_customer.id)
...
class TestConversationCRUD:
    ...
    @pytest.mark.asyncio
    async def test_create_conversation(self, session: AsyncSession, test_customer: Customer):
    ...
    @pytest.mark.asyncio
    async def test_get_conversation(self, session: AsyncSession, test_conversation: Conversation):
        """Test retrieving conversation by ID."""
        conversation = await get_conversation(session, test_conversation.id)

        assert conversation is not None
        assert conversation.id == test_conversation.id

    @pytest.mark.asyncio
    async def test_update_conversation_status(self, session: AsyncSession, test_conversation: Conversation):
    ...
    async def test_list_customer_conversations(
        self, session: AsyncSession, test_customer: Customer, test_conversation: Conversation
    ):
...
class TestMessageCRUD:
    ...
    @pytest.mark.asyncio
    async def test_create_message(self, session: AsyncSession, test_conversation: Conversation):
    ...
    async def test_get_latest_message(
        self, session: AsyncSession, test_conversation: Conversation, test_message_agent: Message
    ):
        """Test retrieving the most recent message."""
        latest = await get_latest_message(session, test_conversation.id)
    ...
    @pytest.mark.asyncio
    async def test_delete_message(self, session: AsyncSession, test_message_customer: Message):
        """Test deleting a message."""
        deleted = await delete_message(session, test_message_customer.id)
...
class TestTicketCRUD:
    ...
    async def test_create_ticket(
        self, session: AsyncSession, test_conversation: Conversation, test_customer: Customer
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_ticket(self, session: AsyncSession, test_ticket: Ticket):
        """Test retrieving ticket by ID."""
        ticket = await get_ticket(session, test_ticket.id)

        assert ticket is not None
        assert ticket.id == test_ticket.id

    @pytest.mark.asyncio
    async def test_update_ticket(self, session: AsyncSession, test_ticket: Ticket):
    ...
    async def test_list_conversation_tickets(
        self, session: AsyncSession, test_conversation: Conversation, test_ticket: Ticket
    ):
        """Test listing all tickets for a conversation."""
        tickets = await list_conversation_tickets(session, test_conversation.id)
...
class TestKnowledgeBaseCRUD:
    ...
    @pytest.mark.asyncio
    async def test_create_knowledge_base_entry(self, session: AsyncSession):
    ...
    @pytest.mark.asyncio
    async def test_search_knowledge_base(self, session: AsyncSession, test_knowledge_article: KnowledgeBase):
    ...
    @pytest.mark.asyncio
    async def test_get_knowledge_base_entry(self, session: AsyncSession, test_knowledge_article: KnowledgeBase):
        """Test retrieving knowledge base entry by ID."""
        entry = await get_knowledge_base_entry(session, test_knowledge_article.id)

        assert entry is not None
        assert entry.id == test_knowledge_article.id

    @pytest.mark.asyncio
    async def test_update_knowledge_base_entry(self, session: AsyncSession, test_knowledge_article: KnowledgeBase):
    ...
    @pytest.mark.asyncio
    async def test_delete_knowledge_base_entry(self, session: AsyncSession, test_knowledge_article: KnowledgeBase):
        """Test deleting knowledge base entry."""
        deleted = await delete_knowledge_base_entry(
            session, test_knowledge_article.id
        )
