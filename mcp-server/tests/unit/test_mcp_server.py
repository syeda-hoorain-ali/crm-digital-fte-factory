...
async def test_call_identify_customer_tool_existing_customer(client_session: ClientSession):
    """Test identify_customer finds existing customer (one with a ticket)."""
    import uuid
    email = f"test_{uuid.uuid4().hex[:8]}@example.com"

    # Create customer by creating a ticket first
    ticket_result = await client_session.call_tool("create_ticket", {
        "customer_id": email,
        "issue": "Test issue",
        "priority": "normal",
        "channel": "web_form"
    ...
    assert ticket_result.isError is False

    # Now identify the customer - should find them in tickets
    result = await client_session.call_tool("identify_customer", {
        "email": email
    })
    assert result is not None
